...
**分发渠道多样，统计覆盖有限**  
- 基础软件的适配成果主要通过上游社区回馈，分发方式跟随各根社区，数据难以统一追踪。  
...
### 2. RuyiSDK 遥测设备上报数
**定义**：RuyiSDK 包管理器向后端 `ruyi-backend` 上报的唯一设备标识符（`installation_id`，UUID）的去重数量。  

**UUID 生命周期**：每个操作系统实例（用户 home 目录）首次运行 `ruyi` 时生成并持久化于 `~/.cache/ruyi/telemetry/installation.json`；重装系统、更换硬盘或刷写整机镜像后，原 UUID 丢失，新环境生成新 UUID，因此同一物理设备可能被计为多台设备。  

**隐私保护**：遥测载荷不含个人身份信息、IP 地址或地理位置，仅包含架构/OS/libc/shell 类型、RISC-V 板信息、ruyi 版本及子命令调用模式（不含参数）等匿名数据。  

**上报策略**：  
- 自 0.42.0 起，默认遥测模式为 `local`，即仅本地记录事件，不自动上传；  
- 0.23–0.41 版本默认模式为 `on`，会周期自动上传遥测数据；  
- 用户可通过设置环境变量 `RUYI_TELEMETRY_OPTOUT=1`（或配置 `[telemetry] mode = "off"`）完全禁用遥测，此时仅做一次性版本号上报，不生成持久化 UUID。  

> 📌 **局限性**：  
> - 用户可自愿禁用遥测，禁用后不会被计入设备上报数；  
> - 同一物理设备因重装系统或多安装实例可能产生多个 UUID，导致设备数高估。
