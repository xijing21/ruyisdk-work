# RuyiSDK 包管理器自动化发版与测试流程优化

## 一、 当前发布流程现状
当前 `ruyi` 包管理器的发布与测试流程呈现出"构建发版与测试报告生成高度自动化，新特性测试与分发推广仍高度依赖人工、且存在滞后割裂"的特点。
### 1. 当前流程图
```mermaid
...
flowchart TD
    ...
    subgraph 新特性人工测试与报告评估
    Release --> ManualChk[测试人员: 基于 changelog 分析新特性]
    ManualChk --> ManualTest[测试人员: 针对新特性执行人工测试]
    
    subgraph 测试报告生成与完善
    AutoGenReport[自动: 生成 Gitee Markdown 测试报告] --> ManualFill[人工: 填写测试日期 + 检查 Issue 处理情况]
    end
    
    ManualTest --> AutoGenReport
    ManualFill --> ManualReview[测试团队: 人工评审报告并决定是否对外推广]
    end
    subgraph 人工分发与推广
    ManualReview -->|确认无问题| ManualDist[人工: 多渠道同步包及发版新闻推送]
    end
    subgraph 测试用例滞后沉淀
    ManualFill -->|发版后| AutoAdd[测试人员: 将新特性测试用例补充至自动化套件]
    ...
    class CI_Lint,AutoTag,ReleaseBuild,Release,PyPI,AutoGenReport auto;
    class ManualChk,ManualTest,ManualFill,ManualReview,ManualDist manual;
    class AutoAdd lag;
```
### 2. 流程描述与自动化程度说明
*   **全自动化环节（绿色）**：
    ...
    *   **基础渠道发版**：自动创建 GitHub Release 页面，并将 Wheel 包发布到 PyPI（此动作发生在人工新特性测试之前）。
    *   **测试报告自动生成**：在“测试报告生成与完善”环节，系统先自动生成 Gitee Markdown 测试报告主体内容，不再需要测试人员逐条手工撰写。
*   **纯人工环节（红色）**：
    *   **新特性人工测试**：发版后，测试人员需根据自动生成的 changelog 分析本次引入的新特性，并对其进行人工手动测试。
    *   **测试报告完善与评审**：在系统自动生成报告主体后，测试人员仅需手工填写测试日期、并检查仓库 Issue 处理情况，随后交由团队人工评审，以此作为对外推广的质量依据。
...
---
## 二、 优化后的发布流程（全链路自动化方案）
针对现状中仍存在的人工瓶颈（新特性人工测试、报告评审、分发推广），设计以下全链路自动化优化方案，通过引入结构化测试产物、AI 内容生成与审批门禁，实现"测试-报告-评审-分发-推广"的闭环自动化。
### 1. 优化后流程图
```mermaid
%%{init: {'themeVariables': {'clusterBkg': '#f0f0f0', 'clusterBorder': '#cccccc'}}}%%
flowchart LR
    ...
    subgraph 报告与裁决层
    A4 --> B1["CI: report-generator 聚合 JUnit+Issues<br/>(现状已部分实现，进一步消除日期/Issue人工填写)"]
...
```
### 2. 方案核心价值
*   **测试报告进一步自动化**：在现状"自动生成报告 + 人工填日期/检查 Issue"基础上，进一步将测试日期、Issue 处理情况纳入自动拉取，实现报告端到端无人值守，测试结果实时归档。
...
**问题剖析**：
自动化流程极大地保障了历史功能的回归测试效率，测试报告也已实现自动生成，但面临新特性覆盖的问题。如果发版修改的代码引入了新功能，而开发者未同步补充单元测试和集成测试用例，现有的 CI 流水线依然会因"未覆盖到新代码"而通过。此时，关于新特性的发版质量是由后续的**新特性人工测试环节**来补位。这虽然保障了质量，但导致流程增加了大量的人工干预和介入，拉长了发布周期，未能充分发挥自动化的效能。
...
### 问题2：发版依据缺失及测试与研发割裂
**问题剖析**：
当前流程中，发版规划代码修改的前置需求依据相对缺失。受既往"小步快跑"的发版策略影响，实际操作演变成了"滞后测试"模式：先打 Release，再根据 changelog 分析是否需要新增测试用例，人工测试通过后才算完成发版。这种模式导致测试用例开发与代码研发割裂，拉长了有效验证周期。
...
1.  **轻量化发布计划管理（基于 GitHub Milestone）**：
    利用 GitHub 现有的 Milestone 功能管理发布计划。每个 Milestone 代表一个待发版本，明确纳入需解决的 Issue，并标注大致的时间范围（如"预计X月上旬"）。当 Milestone 内规划的核心 Issue 均已开发完成并合并时，即视为满足发版条件。（ruyi 仓库已经定义Milestone，但是Milestone管理还需要加强，Milestone纳入需要解决的issue需要加强，Release时交付的内容是否与Milestone规划匹配没有check；没有依据Milestone规划和代码变动在发版测试前完成测试用例；）
2.  **PR 级别的测试左移卡点**：
